...
def _departure_rows(
    departures: list[dict[str, Any]], *, absolute_time: bool = False
) -> str:
    rows = []
    for item in departures:
        ...
        if absolute_time and not item.get("cancelled"):
            display_time_value = item.get("time", item.get("display_time", ""))
        else:
            display_time_value = item.get("display_time", item.get("time", ""))
        display_time = html.escape(str(display_time_value))
        countdown = (
            ""
            if absolute_time or item.get("time_mode") == "departure_time"
...
:root{{color-scheme:dark;font-family:system-ui,sans-serif;background:#0b1220;color:#f4f7fb}}body{{margin:0;background:linear-gradient(135deg,#0b1220,#17233b);min-height:100vh}}main{{max-width:980px;margin:auto;padding:24px 16px 48px}}a{{color:#8bd3ff}}h1{{margin:0 0 8px;font-size:clamp(2rem,6vw,4rem)}}.subtle{{color:#aab8cb}}.toolbar{{display:flex;justify-content:space-between;gap:16px;align-items:center;flex-wrap:wrap;margin:20px 0}}.board{{background:#05080e;border:1px solid #324057;border-radius:18px;overflow:hidden;box-shadow:0 18px 50px #0006}}.row{{display:grid;grid-template-columns:76px minmax(0,1fr) 100px;gap:16px;align-items:center;padding:18px 22px;border-bottom:1px solid #202a3b}}.row:last-child{{border:0}}.line{{display:inline-block;min-width:42px;padding:2px 8px;text-align:center;font-size:1.35rem;font-weight:800;line-height:1.2}}.destination{{font-size:1.2rem;overflow-wrap:anywhere}}.time{{text-align:right;font-size:1.8rem;font-variant-numeric:tabular-nums}}.time small{{font-size:.65em;color:#aab8cb}}.station{{color:#8bd3ff;font-size:.8rem}}.delay{{color:#ff9d66;font-size:.85rem}}.cancelled{{color:#ff6b7a;text-decoration:line-through}}.status,.grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:10px;margin:16px 0}}.status div,.card{{background:#121c2d;border:1px solid #324057;border-radius:14px;padding:20px}}.status strong{{display:block;font-size:1.15rem;margin-top:4px}}.setting{{min-width:0}}.control-row,.station-search{{display:flex;gap:8px;align-items:center;flex-wrap:wrap}}.control-row input,.control-row select{{flex:1;min-width:0}}.reset{{background:#26344a;border-color:#53627a;font-size:.85rem}}.station-card{{border:1px solid #53627a;border-radius:12px;padding:16px;margin:14px 0}}.station-heading{{display:flex;justify-content:space-between;align-items:center;gap:12px}}.route-row{{display:grid;grid-template-columns:1fr 2fr auto;gap:8px;margin:8px 0}}.station-search{{margin-top:10px}}.station-search select{{min-width:280px;flex:1}}button,input,textarea,select{{font:inherit;border-radius:9px;border:1px solid #53627a;padding:10px 12px;background:#101a2b;color:inherit}}button{{cursor:pointer;background:#207bb3;border-color:#65c6ff;font-weight:700}}button:disabled{{opacity:.55;cursor:wait}}label{{display:block;margin:14px 0 6px;font-weight:700}}.card{{margin-top:18px}}.danger{{background:#7e2632;border-color:#ff8793;margin-left:8px}}.notice{{padding:14px 16px;border-radius:10px;background:#3b2913;color:#ffdca6;margin:16px 0}}.ok{{color:#8ee6ad}}.error{{color:#ff9aa5}}.empty{{padding:42px 22px;text-align:center;color:#aab8cb}}.spinner{{display:inline-block;width:14px;height:14px;border:2px solid #53627a;border-top-color:#8bd3ff;border-radius:50%;animation:spin .8s linear infinite;vertical-align:-2px}}@keyframes spin{{to{{transform:rotate(360deg)}}}}details.help-box{{margin-top:12px}}code{{overflow-wrap:anywhere}}.display-page main{{max-width:1320px;min-height:100vh;box-sizing:border-box;display:flex;flex-direction:column;justify-content:center;padding:clamp(16px,4vw,48px)}}.display-page .display-shell{{width:100%}}.display-page .display-toolbar{{margin:0 0 clamp(16px,3vw,28px)}}.display-page .display-toolbar h1{{font-size:clamp(2rem,5vw,4.5rem)}}.display-page .display-board{{border-radius:clamp(12px,2vw,22px)}}.display-page .row{{grid-template-columns:minmax(64px,8vw) minmax(0,1fr) minmax(96px,12vw);padding:clamp(14px,2.4vw,28px) clamp(16px,3vw,34px);gap:clamp(10px,2vw,28px)}}.display-page .line{{font-size:clamp(1.35rem,3vw,2.4rem);min-width:clamp(42px,5vw,72px)}}.display-page .destination{{font-size:clamp(1.15rem,2.7vw,2rem)}}.display-page .time{{font-size:clamp(1.5rem,3.4vw,3rem)}}.display-page .station{{font-size:clamp(.75rem,1.4vw,1rem)}}.display-page .display-refresh{{margin:16px 0 0;text-align:center;font-size:.85rem}}.display-exit{{font-size:clamp(.95rem,1.5vw,1.2rem)}}@media(max-width:600px){{.display-page main{{padding:12px 8px 24px;justify-content:flex-start}}.display-page .display-toolbar{{margin-bottom:14px}}.display-page .row{{grid-template-columns:minmax(48px,16vw) minmax(0,1fr) minmax(72px,22vw);gap:8px;padding:12px 10px}}.display-page .line{{min-width:32px;padding:2px 4px;font-size:1.05rem}}.display-page .destination{{font-size:1rem}}.display-page .time{{font-size:1.2rem}}.display-page .station{{font-size:.68rem}}.display-page .display-board{{border-radius:12px}}}}{line_style_css()}
 </style><style>@media(max-width:600px){{.route-row{{grid-template-columns:1fr}}.route-row button{{width:100%}}}}[hidden]{{display:none !important}}.station-search{{flex-direction:column;align-items:stretch}}.station-search select{{min-width:0;width:100%}}.setup-guide{{margin:12px 0 18px;padding:16px 18px;border:1px solid #385875;border-radius:12px;background:linear-gradient(135deg,#142b42,#102033)}}.setup-guide strong{{display:block;margin-bottom:8px}}.setup-guide ol{{margin:0;padding-left:22px;color:#c4d2e3}}.setup-guide li+li{{margin-top:6px}}.line-picker{{margin-top:16px;padding:16px;border:1px solid #385875;border-radius:12px;background:#0e192a}}.line-picker-header{{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}}.line-picker-header h4{{margin:0 0 4px}}.line-picker-help{{margin:0}}.step-pill{{display:inline-flex;align-items:center;white-space:nowrap;padding:6px 10px;border:1px solid #4b7190;border-radius:999px;background:#172b43;color:#bfe7ff;font-size:.82rem;font-weight:700}}.line-picker-actions{{margin:14px 0 0}}.line-filter-row{{display:flex;gap:8px;align-items:center;margin-top:12px}}.line-filter-row input{{flex:1;min-width:180px}}.line-count{{color:#aab8cb;font-size:.9rem}}.line-options{{display:grid;gap:12px;margin-top:14px}}.line-group{{padding:12px;border:1px solid #2e4058;border-radius:10px;background:#111f32}}.line-group-heading{{display:flex;justify-content:space-between;gap:8px;align-items:center;margin-bottom:8px;color:#dbeeff;font-size:.9rem}}.line-group-options{{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px}}.line-option{{display:flex;align-items:center;gap:10px;margin:0;padding:10px 12px;border:1px solid #3a4b64;border-radius:9px;background:#101a2b;color:inherit;cursor:pointer;transition:border-color .15s,background .15s,transform .15s}}.line-option:hover{{border-color:#65c6ff;background:#14253a;transform:translateY(-1px)}}.line-option.is-selected{{border-color:#65c6ff;background:#173a58;box-shadow:0 0 0 1px #65c6ff33}}.line-option input{{flex:0 0 auto;width:18px;height:18px;accent-color:#65c6ff}}.line-option-main{{display:flex;flex-direction:column;gap:2px;min-width:0}}.line-option-name{{font-weight:800;overflow-wrap:anywhere}}.line-option-meta{{color:#aab8cb;font-size:.82rem;overflow-wrap:anywhere}}.route-config{{margin-top:12px;padding:12px;border:1px solid #385875;border-radius:10px;background:#14253a}}.route-config .subtle{{display:block;margin:4px 0 8px}}</style></head><body{body_attribute}><main>{content}</main></body></html>""".encode()
...
class WebApplication:
    ...
    def display(self) -> bytes:
        try:
            ...
            departures, error = [], str(exc)
        rows = _departure_rows(departures, absolute_time=True)
        message = f'<div class="notice">{html.escape(error)}</div>' if error else ""
    ...
    def settings(
        ...
        descriptions = {
            ...
            "display.time_mode": (
                "Minuten bis Abfahrt oder konkrete Abfahrtszeit; gilt für "
                "Hardware-Display und Dashboard. Der reine Web-Displaymodus "
                "zeigt immer die absolute Abfahrtszeit."
            ),
...
<div class="line-picker" data-line-picker data-selected-lines="{selected_lines_json}"><div class="line-picker-header"><div><h4>Linien und Ziele</h4><p class="subtle line-picker-help">Linien laden, auswählen und danach optional eine Richtung oder Zielstation festlegen.</p></div><span class="step-pill" data-line-status>{'2 · Linien laden' if valid else '1 · Haltestelle auswählen'}</span></div><div class="control-row line-picker-actions"><button type="button" data-load-lines onclick="loadLines(this)" {"disabled" if not valid else ""}>Verfügbare Linien laden</button><span class="subtle" data-lines-message>{'Schritt 2: Verfügbare Linien laden.' if valid else 'Schritt 1: Erst einen Geofox-Treffer auswählen.'}</span></div><div class="line-filter-row" data-line-filter-row hidden><input type="search" data-line-search placeholder="Linie suchen, z. B. 124 oder U2 …" autocomplete="off" aria-label="Linien filtern"><span class="line-count" data-line-count></span></div><div class="line-options" data-line-options role="group" aria-label="Verfügbare Linien"></div><div data-route-configs></div></div>{routes_error_markup}<div data-routes>{routes}</div><details><summary>Legacy-Konfiguration manuell bearbeiten</summary><p class="subtle">Bestehende Bus-Konfigurationen bleiben kompatibel. Für neue Haltestellen bitte die Geofox-Linienauswahl verwenden.</p><button type="button" onclick="addRoute(this)">Route hinzufügen</button></details></article>'''
...
<section class="card"><div class="station-heading"><div><h2>Haltestellen und Linien</h2><p class="subtle">Haltestelle eintippen, Geofox-Vorschlag auswählen; Name, Stadt und ID werden automatisch übernommen.</p></div><button type="button" onclick="addStation()">Haltestelle hinzufügen</button></div><div class="setup-guide"><strong>So richtest du eine Haltestelle ein</strong><ol><li>Name eingeben und einen Geofox-Treffer auswählen. Stadt, Geofox-ID und Kürzel werden automatisch ergänzt.</li><li>Auf „Verfügbare Linien laden“ klicken und die gewünschten Linien markieren.</li><li>Bei Bedarf Richtung oder Zielstation festlegen und anschließend unten „Speichern und prüfen“ wählen.</li></ol></div><div id="stations">{stations}</div></section>
...
function updateLinePicker(card){{const options=[...card.querySelectorAll('[data-line-option]')];const selected=options.filter(input=>input.checked).length;const filterRow=card.querySelector('[data-line-filter-row]');const count=card.querySelector('[data-line-count]');const status=card.querySelector('[data-line-status]');const hasLines=options.length>0;if(filterRow)filterRow.hidden=!hasLines;if(count)count.textContent=hasLines?selected+' von '+options.length+' ausgewählt':'';if(status)status.textContent=card.dataset.valid!=='true'?'1 · Haltestelle auswählen':!hasLines?'2 · Linien laden':selected?'4 · Richtung/Ziel prüfen':'3 · Linien auswählen'}}
function filterLines(input){{const card=input.closest('[data-station]');const query=input.value.trim().toLocaleLowerCase('de-DE');card.querySelectorAll('[data-line-option]').forEach(option=>{{const visible=!query||option.closest('.line-option').textContent.toLocaleLowerCase('de-DE').includes(query);option.closest('.line-option').hidden=!visible}});card.querySelectorAll('[data-line-group]').forEach(group=>{{group.hidden=!Array.from(group.querySelectorAll('.line-option')).some(option=>!option.hidden)}});updateLinePicker(card)}}
function invalidateStation(card){{card.dataset.valid='false';card.querySelector('[data-station-field="id"]').value='';card.querySelector('[data-station-field="serviceTypes"]').value='[]';card.querySelector('[data-station-search]').hidden=true;card.querySelector('[data-station-results]').replaceChildren();card.querySelector('[data-search-message]').textContent='';card.querySelector('[data-line-options]').replaceChildren();card.querySelector('[data-route-configs]').replaceChildren();card.querySelector('[data-lines-message]').textContent='Schritt 1: Erst einen Geofox-Treffer auswählen.';card.querySelector('[data-load-lines]').disabled=true;lineControllers.get(card)?.abort();lineSequences.set(card,(lineSequences.get(card)||0)+1);card.querySelector('[data-routes]').replaceChildren();card.querySelector('[data-line-picker]').dataset.selectedLines='[]';updateLinePicker(card)}}
function bindStation(card){{const name=card.querySelector('[data-station-field="name"]');const city=card.querySelector('[data-station-field="city"]');const select=card.querySelector('[data-station-results]');const search=card.querySelector('[data-line-search]');[name,city].forEach(input=>input.addEventListener('input',()=>{{invalidateStation(card);scheduleStationSearch(card)}}));select.addEventListener('change',()=>applyStation(select));search?.addEventListener('input',()=>filterLines(search));}}
...
function defaultStationLabel(card,name){{const used=new Set([...document.querySelectorAll('[data-station-field="label"]')].filter(input=>input.closest('[data-station]')!==card).map(input=>input.value.trim().toUpperCase()));const letters=name.normalize('NFD').replace(/[\u0300-\u036f]/g,'').toUpperCase().replace(/[^A-Z0-9]/g,'');for(let length=1;length<=3;length++){{const candidate=letters.slice(0,length);if(candidate&&!used.has(candidate))return candidate}}return letters.slice(0,3)}}
async function searchStation(card){{const query=card.querySelector('[data-station-field="name"]').value.trim();const city=card.querySelector('[data-station-field="city"]').value.trim();const searchBox=card.querySelector('[data-station-search]');const select=card.querySelector('[data-station-results]');const message=card.querySelector('[data-search-message]');const sequence=++requestSequence;controllers.get(card)?.abort();const controller=new AbortController();controllers.set(card,controller);select.disabled=true;message.innerHTML='<span class="spinner" aria-hidden="true"></span> Geofox wird abgefragt …';searchBox.hidden=false;try{{const response=await fetch('/api/stations?q='+encodeURIComponent(query)+'&city='+encodeURIComponent(city),{{signal:controller.signal}});const data=await response.json();if(sequence!==requestSequence)return;if(!response.ok)throw new Error(data.error||'Geofox-Suche fehlgeschlagen');select.replaceChildren();data.stations.forEach(station=>{{const option=new Option(station.combinedName,JSON.stringify(station));select.appendChild(option)}});select.selectedIndex=-1;message.textContent=data.stations.length?'':'Keine passende Haltestelle gefunden.';searchBox.hidden=!data.stations.length}}catch(error){{if(error.name!=='AbortError')message.textContent=error.message}}finally{{if(sequence===requestSequence)select.disabled=false}}}}
function applyStation(select){{if(!select.value)return;const station=JSON.parse(select.value);const card=select.closest('[data-station]');card.querySelector('[data-station-field="name"]').value=station.name;card.querySelector('[data-station-field="city"]').value=station.city;card.querySelector('[data-station-field="id"]').value=station.id;card.querySelector('[data-station-field="serviceTypes"]').value=JSON.stringify(station.serviceTypes||[]);const label=card.querySelector('[data-station-field="label"]');if(!label.value.trim())label.value=defaultStationLabel(card,station.name);card.dataset.valid='true';card.querySelector('[data-search-message]').innerHTML='<span class="ok">✓ Geofox-Haltestelle ausgewählt</span>';card.querySelector('[data-load-lines]').disabled=false;card.querySelector('[data-lines-message]').textContent='Schritt 2: Verfügbare Linien laden.';card.querySelector('[data-routes]').replaceChildren();updateLinePicker(card)}}
function addStation(){{const first=document.querySelector('[data-station]');const clone=first.cloneNode(true);clone.querySelectorAll('input').forEach(input=>input.value=input.dataset.stationField==='city'?'Hamburg':(input.dataset.stationField==='serviceTypes'?'[]':''));clone.querySelector('[data-routes]').replaceChildren();clone.querySelector('[data-line-options]').replaceChildren();clone.querySelector('[data-route-configs]').replaceChildren();clone.querySelector('[data-line-picker]').dataset.selectedLines='[]';clone.querySelector('[data-station-results]').replaceChildren();invalidateStation(clone);document.getElementById('stations').appendChild(clone);bindStation(clone)}}
...
function renderLineOptions(card, lines){{
    ...
    lines.forEach(line=>{{
        const groupName=line.productLabel||'Weitere Linien';
        let group=groups.get(groupName);
        if(!group){{
            group=document.createElement('section');
            group.className='line-group';
            group.dataset.lineGroup='true';
            ...
            options.className='line-group-options';
            heading.append(title,total);
            group.append(heading,options);
            box.append(group);
            group={{element:group,options,total}};
        ...
        name.className='line-option-name';
        name.textContent=line.name;
        const meta=document.createElement('span');
        meta.className='line-option-meta';
        meta.textContent=[line.productLabel,line.carrier].filter(Boolean).join(' · ');
        main.append(name,meta);
        const syncSelection=()=>{{label.classList.toggle('is-selected',input.checked);updateLinePicker(card)}};
        ...
        if(input.checked){{label.classList.add('is-selected');loadRouteOptions(input)}}
    }});
    groups.forEach(group=>group.total.textContent=group.options.children.length+' Linien');
    updateLinePicker(card);
}}
async function loadLines(button){{const card=button.closest('[data-station]');const stationId=card.querySelector('[data-station-field="id"]').value.trim();const message=card.querySelector('[data-lines-message]');if(!stationId){{message.textContent='Schritt 1: Erst einen Geofox-Treffer auswählen.';return}}const sequence=(lineSequences.get(card)||0)+1;lineSequences.set(card,sequence);lineControllers.get(card)?.abort();const controller=new AbortController();lineControllers.set(card,controller);button.disabled=true;message.innerHTML='<span class="spinner" aria-hidden="true"></span> Linien werden geladen …';try{{const response=await fetch('/api/lines?station_id='+encodeURIComponent(stationId),{{signal:controller.signal}});const data=await response.json();if(lineSequences.get(card)!==sequence)return;if(!response.ok)throw new Error(data.error||'Linien konnten nicht geladen werden');renderLineOptions(card,data.lines);message.textContent=data.lines.length?'Schritt 3: Gewünschte Linien markieren.':'Für diese Haltestelle wurden keine Linien gefunden.';updateLinePicker(card)}}catch(error){{if(error.name!=='AbortError')message.textContent=error.message}}finally{{if(lineSequences.get(card)===sequence)button.disabled=false}}}}
...
document.querySelectorAll('[data-station]').forEach(bindStation);document.getElementById('display.time_mode')?.addEventListener('change',syncTimeDisplayControls);syncTimeDisplayControls();
</script>'''
